 ...
 *
 * Mirrors `NewInvoiceForm.tsx`: client picker, matter picker filtered by
 * selected client, language toggle, valid-until, notes, local-only line-item
 * array. The line items are serialised to a hidden `line_items` JSON field
 * on submit and consumed by `createQuotationAction`.
 *
 * Why local-only line items (not Server Actions per row): the quotation does
 * not exist yet — there is nothing to PATCH against. After the draft is
 * created, the detail page is read-only (drafts can be deleted or
 * mark-sent; line-item edits would require recomputing the totals snapshot
 * stored on the quotations row, which we keep simple by not editing line
 * items post-create for this demo).
 */
...
export function NewQuotationForm({ clients, matters, locale }: Props) {
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <FieldWrap label="Case · Υπόθεση" htmlFor="field-matter_id" required>
          ...
          >
            <option value="" disabled>
              {clientId ? "Select a case" : "Select a client first"}
            </option>
        ...
        <FieldWrap label="Valid until" htmlFor="field-valid_until">
          <input
